...
// 🚀 核心：获取当前登录人的角色 (0:超级管理员, 2:普通管理员)
const currentUserInfo = JSON.parse(localStorage.getItem('userInfo') || '{}')
const myRole = ref(currentUserInfo.role)
...
const handleDelete = (id) => {
  ElMessageBox.confirm('确定要永久删除该账号吗？', '高危操作', {
  ...
  <div class="admin-user-container">
    ...
    <div class="toolbar">
      ...
      <div class="action-area">
        <el-button v-if="myRole === 0" type="success" :icon="Plus" @click="openAddDialog">新增账号</el-button>
      </div>
    </div>
    ...
    <el-table :data="tableData" v-loading="loading" border stripe style="width: 100%; margin-top: 20px;" height="calc(100vh - 240px)">
      ...
      <el-table-column label="角色权限" width="160" align="center">
        <template #default="scope">
          ...
          <el-tag v-else :type="scope.row.role === 2 ? 'warning' : 'info'">
            {{ scope.row.role === 2 ? '普通管理员' : '普通用户' }}
          </el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="账号状态" width="120" align="center">
        <template #default="scope">
          <el-switch 
            ...
            :active-value="1" 
            :inactive-value="0" 
            inline-prompt active-text="正常" inactive-text="封禁"
            style="--el-switch-on-color: #13ce66; --el-switch-off-color: #ff4949"
            :disabled="scope.row.role === 0 || (myRole === 2 && scope.row.role !== 1)" 
            @change="handleUpdate(scope.row)"
          />
        </template>
      </el-table-column>

      <el-table-column label="操作" width="120" align="center" fixed="right" v-if="myRole === 0">
        <template #default="scope">
          <el-button 
    ...
    <el-dialog v-model="dialogVisible" title="新增账号" width="500px" destroy-on-close>
      <el-form :model="form" :rules="rules" ref="formRef" label-width="80px">
        <el-form-item label="用户名" prop="username">
          <el-input v-model="form.username" placeholder="请输入唯一用户名" />
        </el-form-item>
        ...
        <el-form-item label="初始状态">
          <el-switch v-model="form.status" :active-value="1" :inactive-value="0" active-text="正常" inactive-text="封禁" />
        </el-form-item>
      </el-form>
      <template #footer>
